    ...
    <div class="container mx-auto ">
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-5 mb-6">
            <h3 class="text-lg font-medium text-gray-700 mb-4 flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 text-[#a53692]" fill="none"
        ...
        @endif
  <!-- Doctors Table - Progressive Enhancement -->
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th
                                    class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden sm:table-cell">
                                    Department
                                </th>
                                <th
                                    class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">
                                    Fee
                                </th>
                                <th
                                    class="px-4 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($doctors as $doctor)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 md:px-6 py-4">
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-8 w-8 md:h-10 md:w-10">
                                                ...
                                                @else
                                                    <div class="h-8 w-8 md:h-10 md:w-10 rounded-full flex items-center justify-center text-white font-medium text-xs md:text-sm"
                                    ...
                                    <td
                                        class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-900 hidden sm:table-cell">
                                        {{ $doctor->department ? $doctor->department->name : 'Not Assigned' }}
                                    </td>
                                    <td
                                        class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-900 hidden md:table-cell">
                                        ${{ number_format($doctor->fee, 2) }}
                                    </td>
                                    <td class="px-4 md:px-6 py-4 whitespace-nowrap">
                                        ...
                                        {{ $doctor->status
                                            ...
                                            <span
                                                class="hidden xs:inline">{{ $doctor->status ? 'Active' : 'Inactive' }}</span>
                                            <span class="xs:hidden">{{ $doctor->status ? 'Act' : 'Inact' }}</span>
                                        </button>
                                    </td>
                                    <td class="px-2 md:px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="">
                                            <a href="{{ route('admin.doctors.edit', $doctor->id) }}" 
                                       class="text-purple-600 hover:text-purple-900 transition-colors text-xs md:text-sm">
                                        Edit
                                    </a>
                                    <button wire:click="confirmDelete({{ $doctor->id }})" 
                                            class="text-red-600 hover:text-red-900 transition-colors text-xs md:text-sm">
                                        Delete
                                    </button>
                                        </div>
                                    </td>
